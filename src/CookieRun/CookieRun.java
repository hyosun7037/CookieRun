...
//	private ImageIcon cookie = new ImageIcon("img/cookieTest.png"); 
	private ImageIcon cookie = new ImageIcon("img/player_origin.gif"); // 기본
	
//	private ImageIcon cookieDown = new ImageIcon("img/slideTest.png"); 
	private ImageIcon cookieDown = new ImageIcon("img/player_down.gif"); // 슬라이딩
	
//	private ImageIcon cookieJump = new ImageIcon("img/jumpTest.png");
	private ImageIcon cookieJump = new ImageIcon("img/player_up.gif"); // 점프
	
//	private ImageIcon cookieDJump = new ImageIcon("img/doubleJumpTest.png");
	private ImageIcon cookieDJump = new ImageIcon("img/player_doubleUp.gif"); // 더블점프
	
//	private ImageIcon cookieFall = new ImageIcon("img/fallTest.png");
	private ImageIcon cookieFall = new ImageIcon("img/player_jumpEnd.png"); // 낙하
	
//	private ImageIcon cookieHit = new ImageIcon("img/hitTest.png");
	private ImageIcon cookieHit = new ImageIcon("img/player_attack.png"); // 공격받았을 때
		...
		public MyPanel() {
			...
			for (int i = 0; i < maxX; i += 2) { // 발판은 4칸을 차지하는 공간, 2,2사이즈로 반복문
				for (int j = 0; j < maxY; j += 2) { // 색 값이 0일 경우 (검은색)
					if (colorArr[i][j] == 0) {
						fieldList.add(new Field(fieldIc1.getImage(), i * 40, j * 40, 80, 80)); // 좌표에 40을 곱하고, 넓이와 높이는
																								// 80
			...
			for (int i = 0; i < maxX; i += 2) { // 발판은 4칸을 차지하는 공간, 2,2사이즈로 반복문
				for (int j = 0; j < maxY; j += 2) { // 색 값이 0일 경우 (검은색)
					if (colorArr[i][j] == 6579300) {
						fieldList.add(new Field(fieldIc2.getImage(), i * 40, j * 40, 80, 80)); // 좌표에 40을 곱하고, 넓이와 높이는
																								// 80
...
//			}
			...
			this.addKeyListener(new KeyAdapter() {
				...
				public void keyPressed(KeyEvent e) {
					...
					if (!escKeyOn) { // esc가 없을 때에만 작동
						...
						if (e.getKeyCode() == KeyEvent.VK_DOWN) {
							...
							if (c1.getImage() != cookieDown.getImage() && !c1.isJump() && !c1.isFall()) {
				...
				public void keyReleased(KeyEvent e) {
					if (e.getKeyCode() == KeyEvent.VK_DOWN) {
						...
						if (c1.getImage() != cookie.getImage() // 쿠키이미지가 기본이미지 X
								&& !c1.isJump() // 점프중 X
								&& !c1.isFall()) { // 낙하중 X
		...
		protected void paintComponent(Graphics g) {
			...
			for (int i = 0; i < fieldList.size(); i++) {
				...
				if (tempFoot.getX() > -90) {
					...
					buffg.drawImage(
							...
							tempFoot.getImage(), tempFoot.getX(), tempFoot.getY(), tempFoot.getWidth(),
			...
			// 쿠키 그리기
			// buffg.drawImage(c1.getImage(), c1.getX(), c1.getY(), c1.getWidth(), c1.getHeight(), null); // 기준
			buffg.drawImage(c1.getImage(), c1.getX() -140, c1.getY() -205, cookie.getImage().getWidth(null)*9/10, cookie.getImage().getHeight(null)*9/10, null);
				...
				while (true) {
					...
					for (int i = 0; i < jellyList.size(); i++) {
						...
						if (tempJelly.getX() < -90) {
							...
							if (c1.getImage() != cookieDown.getImage() && tempJelly.getX() >= c1.getX()
									&& tempJelly.getX() <= face && tempJelly.getY() >= c1.getY() && tempJelly.getY() <= foot
					...
					for (int i = 0; i < tacleList.size(); i++) {
						...
						if (tempTacle.getX() < -90) {
							...
							}else if( // 무적상태 X, 슬라이드 중, 캐릭터 범위 안에 장애물이 있으면
									!c1.isInvincible()
									...
									&& tempTacle.getY() + tempTacle.getHeight() *95/100 > c1.getY())
							{
								hit();
									...
									&& tempTacle.getX() + tempTacle.getWidth() / 2 <=face
									&& tempTacle.getY() < c1.getY()
									&& tempTacle.getY() + tempTacle.getHeight()*95/100 > c1.getY() + c1.getHeight()
									) {
			...
			public void run() {
				while (true) {
					...
					if (!escKeyOn // 일시중지가 발동 X
						...
						if (downKeyOn // 다운키 누르고
								&& !c1.isJump() // 점프상태 X
								&& !c1.isFall() // 낙하상태 X
								&& c1.getImage() != cookieDown.getImage()) { // 쿠키이미지가 슬라이드 이미지가 X

							c1.setImage(cookieDown.getImage()); // 누운 이미지로 변경

						} else if (!downKeyOn // 다운키 누른상태 X
								&& !c1.isJump() // 점프상태 X
								&& !c1.isFall() // 낙하상태 X
